...
def plot_wh_methods():  # from utils.utils import *; plot_wh_methods()
    ...
    h1 = plt.plot(x, ya, '.-', label='yolo method')
    h2 = plt.plot(x, yb ** 2, '.-', label='^2 power method')
    h3 = plt.plot(x, yb ** 2.5, '.-', label='^2.5 power method')
    plt.xlim(left=-4, right=4)
    plt.ylim(bottom=0, top=6)
    plt.xlabel('input')
    plt.ylabel('output')
    plt.legend([h1,h2,h3], ['ya', 'yb**2', 'yb**2.5'])
    fig.tight_layout()
...
def plot_results(output='', start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
